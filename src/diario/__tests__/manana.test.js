// src/diario/__tests__/manana.test.js
...
import { ANIMO, INTENCION, ID_OTRA, MAX_PALABRA_PROPIA } from '@/diario/mananaEmociones'
import { MAX_ANTERIORES, MAX_IDEAS, ideasAnteriores, ideasGenerales } from '@/diario/mananaAcciones'
import { BANCO, MAXIMO_EN_VENTANA, debeAparecer, siguientePregunta } from '@/diario/mananaPausa'
...
describe('criterio 6 — lo escrito antes solo vuelve como dice la regla', () => {
  ...
  it('lo único que vuelve es lo escrito en esta misma pregunta', () => {
    ...
    const modulo = codigoDe('src/diario/mananaAcciones.js')
...
describe('criterio 9 — tres momentos, no una lista de campos', () => {
  ...
  it('el recorrido son cuatro pantallas y cada una es su archivo', () => {
    expect(existsSync('src/components/diario/manana/MomentoInicio.jsx')).toBe(false)
    expect(existsSync('src/components/diario/manana/MomentoAccion.jsx')).toBe(false)
    ;['MomentoAnimo', 'MomentoGratitud', 'MomentoIntencionAccion', 'MomentoPausa'].forEach(
      (nombre) => expect(existsSync(`src/components/diario/manana/${nombre}.jsx`)).toBe(true),
    )
  })
...
describe('§8 — el cierre devuelve lo propio, sin celebrar nada', () => {
  ...
  it('no hay puntuación, ni porcentaje, ni felicitación', () => {
    const apertura = codigoDe('src/components/diario/manana/AperturaDelDia.jsx')
